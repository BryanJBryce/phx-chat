...
Both forms reject unsupported value shapes and NUL characters before database access, and names are limited to 100 Unicode code points. Invalid fields use `aria-invalid` and reference their inline error descriptions. A separate [polite status region](https://www.w3.org/WAI/WCAG22/Techniques/aria/ARIA22) announces how many messages arrived, without replaying the history. Initial loads and duplicate notifications stay quiet; fallback reloads announce only an increase in the displayed message count. The total count makes successive single-message announcements distinct.
