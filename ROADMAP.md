...
Presence follow-up (2026-09-21): reproduced the reported Offline → Online → Offline bounce in private Safari on the port 4001 preview. Server-side mount/Presence tracing showed both original connections disappearing, then a new WebSocket LiveView joining about 1.5 seconds later and remaining for about 13 seconds. Added a capture-phase `pagehide` listener calling the public `LiveSocket.disconnect()` API before Phoenix Socket's handler, which removes the clean-close reload callback and cancels pending reloads. No Presence debounce, manual untracking, or dependency patch was needed.

Verified the fix in native private Safari with two tabs of one disposable identity: connection counts went 2 → 1 → 0, the remaining tab stayed Online after the first closed, and there were no replacement mounts or joins during a 30-second observation after the final close. Refresh retained identity/history; killing only the test tab's WebSocket transport automatically recovered one connection with the same identity/history; navigating away and back recovered the same state. Browser-back behavior passed, though BFCache use was not independently instrumented. `mix assets.build`, `mix precommit` (12 tests), and `mix credo --strict` passed. This regression is documented as a real-browser check rather than a simulated ExUnit lifecycle test.

P2 review follow-up (2026-09-21): fixed all five remaining findings. The two text fields share a small cast/validation boundary that rejects unsupported shapes without retaining unsafe form values, and rejects NUL characters before persistence. Known form events with missing payloads return validation errors. Names now have a documented 100-codepoint limit, including combining-character sequences, preventing unique-index size exceptions. Core inputs associate errors through `aria-invalid`/`aria-describedby`; a separate polite status region reports newly displayed message counts. Initial history and repeated events do not announce, and a fallback reload announces only newly discovered messages. A scalar displayed count is the only additional history bookkeeping.

Added two integrated regression scenarios: HTTP joins exercise malformed form/field shapes, invalid UTF-8, NUL text, an index-sized name, and a long combining sequence, then prove a valid multibyte name at the limit can join/rejoin; this catches exceptions before either safe rendering or persistence. A connected form scenario exercises malformed events and a NUL message, verifies no row/broadcast, then corrects the input and sends successfully; this catches dead LiveViews and unusable error recovery. Existing identity/delivery/order tests now check accessible error association/removal, distinct subsequent announcements, quiet initial history, and duplicate-event handling. Test-critic defaults applied (no project opt-out file), with raw events used deliberately for inputs browser controls cannot normally generate.

Verification: `mix precommit` (14 tests), `mix assets.build`, and `mix credo --strict` passed. In the running browser, blank/overlong names and a NUL message showed associated errors; correction removed invalid attributes; a second independent view exposed the new-message status in its accessibility tree; reload retained history with an empty status region. Actual screen-reader speech was not exercised. The temporary browser identity/message were removed by their own IDs, preserving existing chat data.
